...
export async function applyJob(payload: IApplyJobSchema) {
	try {
		const result = db.transaction(async (tx: any) => {
			const [job, rounds, application, resumeSummary, candidate] = await Promise.all([
...
export async function getApplicationsForJob(payload: IGetApplicationsForJobSchema) {
	try {
		const [applications, rounds] = await Promise.all([getApplicationsByJobIdFromDB(payload.jobId), getRoundsByJobIdFromDB(payload.jobId)]);
