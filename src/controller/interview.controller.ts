...
export async function createAIInterview(payload: { applicationId: string, hrId: string, difficulty: string, questionType: string, jobDescription: string }) {
    try {
        const result = await db.transaction(async (tx: any) => {
            const [application] = await Promise.all([
                getApplicationByIdFromDB(payload.applicationId, tx)
            ])
            // create interview
        ...
        if (error instanceof CreateInterviewInDBError || error instanceof GetApplicationByIdFromDBError || error instanceof GenerateQuestionsServiceError || error instanceof InsertBulkQuestionsInDBError || error instanceof AddApplicationTimelineToDBError) {
            throw error
        }
