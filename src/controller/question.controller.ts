...
export async function nextQuestion(payload: INextQuestionSchema) {
	const updateQuestionInDBWorker = new Worker(path.resolve(__dirname, "../worker/updateQuestionInDB.worker.ts"));
	try {
		...
		if (question.length === 0) {
			...
			return await insertQuestionInDB({
		...
		updateQuestionInDBWorker.postMessage({
			questionId: question[0].questionId,
			isDisplayed: true,
		});
		return { ...question[0], isDisplayed: true };
	} catch (error) {
