...
screeningRoute.post('/fetch/details', async (c) => {
    try {
        ...
        if (error instanceof FetchResumeScreeningDetailsError || error instanceof GetResumeScreeningDetailsFromDBError || error instanceof NotFoundError || error instanceof GenerateResumeFeedbackServiceError || error instanceof InsertRoundResultsToDBError || error instanceof GetRoundResultFromDBError) {
            return c.json({ success: false, message: error.message, error: error.cause }, 400)
        }
        return c.json({ success: false, message: 'Something went wrong' }, 500)
    }
